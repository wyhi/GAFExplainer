...
from shapley import gnn_score, GnnNetsNC2valueFunc, GnnNetsGC2valueFunc
...
class PlotUtils(object):
    ...
    def plot_molecule(self,
        ...
        if self.dataset_name == 'mutag':
            ...
            node_color = ['#F7903D', '#4D85BD', '#59A95A', '#29A329', 'brown', 'darkslategray', '#F0EA00']
    ...
    def get_topk_edges_subgraph(self,
        ...
        thres_index = max(edge_mask.shape[0] - top_k, 0)  # 确定阈值索引
        threshold = float(edge_mask.reshape(-1).sort().values[thres_index])  # 确定阈值的值
    ...
    def plot_soft_edge_mask(self,
        ...
        # edge_mask = torch.FloatTensor(edge_mask).cpu()
...
def calculate_selected_nodes(data, edge_mask, top_k):
    # 设置阈值：第top_k的edge_mask
    threshold = float(edge_mask.reshape(-1).sort(descending=True).values[min(top_k, edge_mask.shape[0] - 1)])
    hard_mask = (edge_mask > threshold).cpu()  # 返回bool类型
    edge_idx_list = torch.where(hard_mask == 1)[0]  # 返回=1处的索引值
    selected_nodes = []
...
class AAFExplainer(nn.Module):
    def __init__(self, model, in_channels: int, dim_node:int,device, explain_graph: bool = True, epochs: int = 80,
                 lr: float = 0.005, coff_size: float = 0.01, coff_ent: float = 5e-4,
                 t0: float = 5.0, t1: float = 1.0, num_hops: Optional[int] = None):
        super(AAFExplainer, self).__init__()
        self.model = model
        self.device = device
        ...
        # training parameters for AAFExplainer
        self.epochs = epochs
        self.lr = lr
        # 计算交叉熵损失函数时要用到的参数
        ...
        # Explanation model in AAFExplainer
        self.elayers = nn.ModuleList()
        self.elayers.append(nn.Sequential(nn.Linear(in_channels, 64), nn.ReLU()))  # nn.BatchNorm1d(64),
        self.elayers.append(nn.Linear(64, 1))
        self.elayers.to(self.device)
        self.hgnn_ac =GNN_AC(in_dim=dim_node, hidden_dim=64, dropout=0.5, activation=F.elu, num_heads=8, cuda=False)
        self.hgnn_ac.to(self.device)
    ...
    def __loss__(self, prob: Tensor, ori_pred: int):
        # 在原始预测标签下新图训练预测值对应的概率
        logit = prob[ori_pred]
        logit = logit + EPS
        pred_loss = - torch.log(logit)

        # size
        # self.sparse_mask_values-->eij ^
    ...
    def explain(self,
        ...
        if self.explain_graph:
            ...
            re_feature = self.hgnn_ac(adj, feature, feature, feature).squeeze()
            # print('embed',embed,'re_feature',re_feature)
            f1 = embed[col] + lamb_a * re_feature[col]
            f2 = embed[row] + lamb_a * re_feature[row]
        ...
        if self.explain_graph:
            with torch.no_grad():
                ...
                for gid in tqdm.tqdm(dataset_indices):
                    ...
                    ori_pred_dict[gid] = logits.argmax(-1).data.cpu()  # int
            ...
            for epoch in range(self.epochs):
                ...
                optimizer.step()
                loss /= count
                etrain_loss.append(loss)
                duration += time.perf_counter() - tic
                print(f'Epoch: {epoch} | Loss: {loss}')
            draw_epoches(np.array(etrain_loss), 'etrain_loss')
        else:
            with torch.no_grad():
                data = dataset[0]
                data.to(self.device)
                ...
                pred_dict = {}
                logits = self.model(data.x, data.edge_index)
                for node_idx in tqdm.tqdm(explain_node_index_list):  # 获取训练集中每个节点的预测标签
                    pred_dict[node_idx] = logits[node_idx].argmax(-1).item()

            # train the mask generator 个人理解是：生成目标子图gs过程就相当于mask生成器的生成过程
            duration = 0.0
            etrain_loss = []
            for epoch in range(self.epochs):
                loss = 0.0
                optimizer.zero_grad()
                tmp = float(self.t0 * np.power(self.t1 / self.t0, epoch / self.epochs))  # 这是甚么？τ
                self.elayers.train()  # 训练解释器层
                tic = time.perf_counter()  # 返回性能计数器的值（以分秒为单位），即具有最高可用分辨率的时钟，以测量短持续时间。
                for iter_idx, node_idx in tqdm.tqdm(enumerate(explain_node_index_list)):
                    ...
                    # 对每一个节点都进行解释，解释的过程就在最小化loss，相当对解释模型进行训练
                    # explain的作用划分出子结构-edge-mask；获得子结构的重要性分数-pred
                    pred, edge_mask = self.explain(x, edge_index, emb, tmp, training=True, node_idx=new_node_index)
                    # 通过优化目标loss 最小化子结构的重要性分数和原结构中的logits之间的差异
                ...
                etrain_loss.append(loss / len(explain_node_index_list))
    ...
    def forward(self,
        ...
        self.__clear_masks__()
        logits = self.model(x, edge_index)
        probs = F.softmax(logits, dim=-1)  # model改过了，所以这一步多余了，但是影响不大
        ...
        if self.explain_graph:
            ...
            sparsity_score = 1 - top_k / len(edge_index[0])
        ...
        pred_mask = [edge_mask]
        related_preds = [{
            'masked': masked_pred,  # 重要节点形成的预测值
            'maskout': maskout_pred,  # 不重要的节点形成的预测
            'origin': probs[label],
            'sparsity': sparsity_score}]
        # print('related_preds[0]', related_preds[0])
