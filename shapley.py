...
def l_shapley(coalition: list, data: Data, local_radius: int,
    ...
    """ shapley value where players are local neighbor nodes 没有近似简化 """
    ...
    set_exclude_masks = []
    set_include_masks = []
    nodes_around = [node for node in local_region if node not in coalition]  # 筛选出不在子图节点范围内的邻居节点
    num_nodes_around = len(nodes_around)

    for subset_len in range(0, num_nodes_around + 1):
        node_exclude_subsets = combinations(nodes_around, subset_len)  # 在nodes_around中选择subset_len个节点 有多个选项
        for node_exclude_subset in node_exclude_subsets:
            set_exclude_mask = np.ones(num_nodes)
            set_exclude_mask[local_region] = 0.0  # 使选择的局部范围都为0
            if node_exclude_subset:
                set_exclude_mask[list(node_exclude_subset)] = 1.0  # 让当前选定的node_exclude_subset中的节点为1
                # set_exclude_mask里面是不在邻域范围内的节点以及在邻域范围内但被剔除的节点为1
                # set_exclude_mask里面是不在邻域范围内的节点以及在邻域范围内但被剔除的节点以及子图中的节点为1
    ...
    num_players = len(nodes_around) + 1  # 增加的一个是子图本身
    num_player_in_set = num_players - 1 + len(coalition) - (1 - exclude_mask).sum(axis=1)  # 邻域内除了exclude节点的节点总个数
    p = num_players
    S = num_player_in_set
...
def mc_shapley(coalition: list, data: Data,
               value_func: str, subgraph_building_method='zero_filling',
               sample_num=1000) -> float:  # sample_num是指采样次数
    """ monte carlo sampling approximation of the shapley value """
    subset_build_func = get_graph_build_func(subgraph_building_method)

    num_nodes = data.num_nodes  # 完整全图的节点个数
    node_indices = np.arange(num_nodes)  # 获取一个索引列表
    coalition_placeholder = num_nodes
    set_exclude_masks = []
    set_include_masks = []

    for example_idx in range(sample_num):
        subset_nodes_from = [node for node in node_indices if
                             node not in coalition]  # subset_nodes_from除了子图中包含的节点之外的节点集合
        random_nodes_permutation = np.array(subset_nodes_from + [coalition_placeholder])  # 增加了一个索引 为啥？
    ...
    mc_shapley_value = marginal_contributions.mean().item()  # 近似方法：直接取平均
